...
import { Activity, Building2, FileText, LayoutDashboard, Receipt, Wrench } from 'lucide-react'
import Header from './Header'
import { useApp } from '../context/AppContext'

export default function Layout() {
  const { household, households, householdId, setActiveHousehold, hasHousehold } = useApp()
  const navItems = [
          ...
          {hasHousehold ? (
            <div className="p-4 bg-teal-900 rounded-xl text-teal-50 text-sm space-y-3">
              ...
              {household && (
                <p className="text-teal-200 text-xs">
                  {household.unit} · {household.address}
                </p>
              )}
            </div>
          ) : (
            <div className="p-4 bg-amber-50 rounded-xl border border-amber-200 text-amber-900 text-sm">
              ...
              <p className="text-amber-800 text-xs mt-1">
                Create one on the Dashboard to add members.
              </p>
            </div>
          )}
        </aside>
